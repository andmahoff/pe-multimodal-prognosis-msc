"""Figure 9 - average precision by model and outcome
(Chapter 4.2). Discrimination is in Figure 7; this is
...
ax.set_xticklabels(["%s\nn = %s, %s events, "
                    ...
                    % (fd.NICE.get(o, o),
                       ...
                       format(int(df[df["outcome"] == o]
                                  ["events"].iloc[0]), ","),
                       float(df[df["outcome"] == o]
                             ["events"].iloc[0])
                       / float(df[df["outcome"] == o]
                               ["n"].iloc[0]))
                    for o in outs], fontsize=7.0)
...
ax.set_title("Average Precision by Model and Outcome",
             fontsize=10.5, pad=30)

fs.save(fig, "fig09_average_precision", "02_Results")
