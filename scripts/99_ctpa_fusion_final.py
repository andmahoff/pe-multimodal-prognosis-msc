...
FEAT = DATA + "/ctpa_factorial_features.csv"
TAG = "both_regex"
OUT = DATA + "/ctpa_fusion_final.csv"
...
for out in OUTS:
    ...
    pg = pd.read_csv(DATA + "/p_ecg_harm_"
                     + out + ".csv")[["hadm_id",
                                      "p_ecg"]]
    pc = pd.read_csv(DATA + "/p_ctpa_" + TAG
                     + "_" + out
                     + ".csv")[["hadm_id",
                                "p_ctpa"]]
    ...
    rows.append({"outcome": out, "n": len(y),
                 "ev": int(y.sum()),
                 ...
                 "wmean3_meta": AM,
                 "inc": t1[0], "lo": t1[1],
                 "hi": t1[2], "vs_meta": t2[0],
                 "vm_lo": t2[1], "vm_hi": t2[2],
                 "r_ehr": r1, "r_ecg": r2})

    ok = (d["both__pe_pos"] == 1).values
    y2 = y[ok]
    if y2.sum() >= 20:
        g2 = grp[ok]
        v2, _ = oof_w([a[ok], b[ok]], y2, g2, G2)
